...
/// Computes and prints a recursive diff between two map (tree) objects.
///
/// Walks both directory trees in parallel, detecting added, deleted,
/// and modified entries. For modified files, a unified-style text diff
/// is printed. For modified directories, the diff recurses.
///
/// # Errors
/// Returns an error if any referenced object cannot be read from storage.
pub fn diff_maps(
    storage: &Storage,
            ...
            (Some(o), Some(n)) => {
                if o.hash != n.hash {
                    if o.mode == "40000" && n.mode == "40000" {
                        ...
                        println!("{} {}", "diff --kitsu".bold(), full_path.bold());
...
/// Prints a line-by-line unified diff between two byte slices.
///
/// Handles binary data gracefully via lossy UTF-8 conversion.
/// Lines are colored: red for deletions, green for insertions.
pub fn print_diff(old_data: &[u8], new_data: &[u8]) {
