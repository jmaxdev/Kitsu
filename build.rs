...
fn main() {
    ...
    let about = std::env::var("CARGO_PKG_DESCRIPTION").unwrap_or_else(|_| "A modern VCS".to_string());

    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let manifest_path = Path::new(&manifest_dir).join("Cargo.toml");
    let manifest_content = fs::read_to_string(manifest_path).expect("Failed to read Cargo.toml");
    let manifest: toml::Value = toml::from_str(&manifest_content).expect("Failed to parse Cargo.toml");
