...
Kitsu is a lightweight, content-addressable version control system designed as a modern, sovereign alternative to Git. Built from scratch in Rust, Kitsu provides snapshot tracking, cryptographic verification, branch/tag primitives, and bidirectional remote synchronization.

Kitsu maintains full interoperability with Git hosting platforms (such as GitHub and GitLab) via an integrated Git bridge, while also supporting sovereign self-hosted registries over SSH/SFTP.
...
- **Dual-Mode Networking**: Seamlessly push to and pull from both Git remotes (via the `kitsu-data` branch) and sovereign SSH/SFTP servers.
...
| **Seal** | Tag / Release | Named semantic version pointer with auto-increment capabilities |
| **Stage** | Index | Staging index tracking file metadata and content hashes |
| **Ignite** | `git init` | Repository creation and configuration wizard |
...
The project is structured as a Cargo workspace with clear separation between core domain logic and presentation layers:
...
└── src/
    ├── core/                   # Core VCS library (kitsu-core engine)
    │   ├── Cargo.toml
    │   └── src/
    ...
    │       └── remote/         # SSH/SFTP transport, GitBridge, and registry
    │
    └── cli/                    # Command-line binary (kitsu)
        ├── Cargo.toml
        └── src/
...
- **C Compiler & Build Tools**:
  ...
  - Windows: Visual Studio C++ Build Tools (included automatically)
...
# 6. Create a semantic version seal
kitsu seal --bump patch

# 7. Push to a configured remote
kitsu push
```
...
|---|---|---|
| `ignite` | None | Initializes a new repository with an optional remote configuration wizard |
| `copy` | `<URL> [DIR]` | Clones a repository from an SSH server or Git remote |
...
| `seal` | `[VERSION] [-b <BUMP>] [-l/--list]` | Creates or lists version seals (major, minor, patch) |
...
| `push` | `[REMOTE] [TARGET]` | Pushes objects and refs to a remote registry |
| `pull` | `[REMOTE] [TARGET]` | Pulls objects and refs from a remote registry |
